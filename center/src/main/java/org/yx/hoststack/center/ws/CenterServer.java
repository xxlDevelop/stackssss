...
//    private final ServiceRegisterConsumer serviceRegisterConsumer;
...
//        channelManager.closeAll();
    ...
    public void initShard() {
        for (int i = 0; i < 10; i++) {
            serverConsistentHash.addShard(String.format("host_stack_center:server_shard_%s", i));
        }
        ;

        for (int j = 0; j < 100; j++) {
            String value = serverConsistentHash.getShard(UUID.randomUUID().toString()).toString();
            System.out.println(value);
        }
        for (int i = 0; i < 50; i++) {
            hostConsistentHash.addShard(String.format("host_stack_center:host_shard_%s", i));
        }
        ;
        for (int i = 0; i < 5; i++) {
            sysModuleConsistentHash.addShard(String.format("host_stack_center:sysModule_shard_%s", i));
        }
        ;
        for (int i = 0; i < 50; i++) {
            gpuInfoConsistentHash.addShard(String.format("host_stack_center:gpuInfo_shard_%s", i));
        }
        ;
        for (int i = 0; i < 50; i++) {
            cpuInfoConsistentHash.addShard(String.format("host_stack_center:cpuInfo_shard_%s", i));
        }
        ;
        for (int i = 0; i < 50; i++) {
            containerConsistentHash.addShard(String.format("host_stack_center:container_shard_%s", i));
        }
        ;
        for (int i = 0; i < 50; i++) {
            agentSessionConsistentHash.addShard(String.format("host_stack_center:agentSession_shard_%s", i));
        }
        ;
    }
}
