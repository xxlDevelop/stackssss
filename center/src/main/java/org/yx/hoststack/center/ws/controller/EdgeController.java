...
public class EdgeController {
    ...
    private void register(ChannelHandlerContext ctx, CommonMessageWrapper.CommonMessage message) {
        try {
            ...
            E2CMessage.E2C_EdgeRegisterReq e2CEdgeRegisterReq = E2CMessage.E2C_EdgeRegisterReq.parseFrom(message.getBody().getPayload());
            Session session = sessionManager.createSession(e2CEdgeRegisterReq.getServiceType().equalsIgnoreCase("RELAY") ? RELAY : IDC, ctx);
            R<?> initR = session.initialize(message);
            if (initR.getCode() == R.ok().getCode()) {
                sessionManager.storeSession(session);
                session.sendMsg(CommonMessageWrapper.CommonMessage.newBuilder()
                                .setHeader(CommonMessageWrapper.CommonMessage.newBuilder()
                                        ...
                                        .setZone(session.getZone())
                                        .setRegion(session.getRegion())
                                ...
                                .setBody(CommonMessageWrapper.Body.newBuilder()
                                        ...
                                        .setPayload(
                                                C2EMessage.C2E_EdgeRegisterResp.newBuilder()
                                                        .setId(session.getNodeId())
                                                        .setHbInterval(serverHbInterval)
                                                        .build().toByteString()
                                        ).build())
                                .build(),
                        () -> KvLogger.instance(this)
                                .p(LogFieldConstants.ERR_MSG, "register success")
                                .i(),
                        () -> {
                            KvLogger.instance(this)
                                    .p(LogFieldConstants.EVENT, CenterEvent.Center_WS_SERVER)
                                    .p(LogFieldConstants.ACTION, "RegisterFailed")
                                    .p("ServiceId", serviceId)
                                    .p("IdcSid", header.getIdcSid())
                                    .p("RelaySid", header.getRelaySid())
                                    .p("ServiceIp", session.getServiceIp())
                                    .i();
                            session.destroy();
                        });
            } else {
                sendErrorResponse(ctx, message, initR.getCode(), initR.getMsg());
            }
        } catch (Exception ex) {
    ...
    private void idcExit(ChannelHandlerContext ctx, CommonMessageWrapper.CommonMessage message) {
        ...
        try {
            ...
            idcSid = exitReq.getIdcSid();
            String finalIdcSid = idcSid;
            sessionManager.getSession(idcSid).ifPresentOrElse(
                    session -> {
                        session.destroy();
                        offline(finalIdcSid);
                    },
...
//            sendSuccessCommonMessage(ctx, message, x00000419);
            KvLogger.instance(this).p(LogFieldConstants.EVENT, CenterEvent.Center_WS_SERVER).p(LogFieldConstants.ACTION, "IDCExit")
                    .p("ServiceId", idcSid)
                    .i();
        } catch (Exception ex) {
            ctx.channel().close();
            KvLogger.instance(this).p(LogFieldConstants.EVENT, CenterEvent.Center_WS_SERVER).p(LogFieldConstants.ACTION, "IDCExitFailed")
                    .p("ServiceId", idcSid)
                    .e();
        }
    }
    ...
    private void ping(ChannelHandlerContext ctx, CommonMessageWrapper.CommonMessage message) {
        try {
            ...
            sessionManager.getSession(serviceId).ifPresentOrElse(
                    session -> {
                        session.tick();
                        session.sendMsg(CommonMessageWrapper.CommonMessage.newBuilder()
                                        .setHeader(CommonMessageWrapper.CommonMessage.newBuilder()
                                ...
                                null,
                                () -> {
                                    //TODO if fail channel is not alive, to be close;
                                    KvLogger.instance(this).p(LogFieldConstants.EVENT, CenterEvent.Center_WS_SERVER).p(LogFieldConstants.ACTION, "IDCHeartBeat")
                                            .p("ServiceId", serviceId)
                                            .i();
                                    session.destroy();
                                });
                    ...
                    () -> {
                        // TODO log get session is null
                        KvLogger.instance(this)
                                .p(LogFieldConstants.EVENT, CenterEvent.Center_WS_SERVER)
                                .p(LogFieldConstants.ACTION, "IDCHeartBeatFaild")
                                .p("ServiceId", serviceId)
                                .p(LogFieldConstants.ERR_MSG, "get session is null")
                                .e();
                        ctx.channel().close();
                    });
...
//            });
            ...
            sendErrorResponse(ctx, message, x00000500.getValue(), x00000500.getMsg(), String.valueOf(ProtoMethodId.Pong.getValue()));
        }
    }
    ...
    private void sendErrorResponse(ChannelHandlerContext ctx, CommonMessageWrapper.CommonMessage message, int code, String msg, String... args) {
        ...
        CommonMessageWrapper.CommonMessage errorMessage = CommonMessageWrapper.CommonMessage.newBuilder()
                .setHeader(CommonMessageWrapper.CommonMessage.newBuilder().getHeaderBuilder()
                        .setMethId(CollectionUtils.isEmpty(params) ? message.getHeader().getMethId() : Integer.parseInt(params.getFirst()))
                        .setLinkSide(CommonMessageWrapper.ENUM_LINK_SIDE.CENTER_TO_EDGE_VALUE)
                        .setProtocolVer(CommonMessageWrapper.ENUM_PROTOCOL_VER.PROTOCOL_V1_VALUE)
                        .setZone(message.getHeader().getZone())
                        .setRegion(message.getHeader().getRegion())
...
//                node.getChannel().disconnect();
                sessionManager.getSession(serviceId).ifPresent(Session::destroy);
            } catch (Exception e) {
